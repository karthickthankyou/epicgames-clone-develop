  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className='mr-4 text-xl text-right uppercase w-28'>
          <div>Page not found</div>
          {showBrokeMessage > 50 && (
            <div className='mt-12 text-xs '>You broke it. ;(</div>
          )}

          {showBrokeMessage > 150 && (
            <div className='mt-3 text-xs '>It is already 404!!!</div>
          )}
          {showBrokeMessage > 300 && (
            <div className='mt-3 text-xs '>Stop doing it!</div>
          )}
        </div>
        <motion.div
          className='px-4 origin-left transform skew-y-12 border border-white rounded-r-3xl text-9xl line-height-2 w-min'
          initial={{ skewY: -6 }}
          animate={{ skewY: 6 }}
